...
def main():
    ...
    if 'migrate' in sys.argv:
        ...
        if not User.objects.filter(username='1').exists():
            User.objects.create_user('1', 'hazcancino@gmail.com', 'Loul')
        
        if not User.objects.filter(username='2').exists():
            User.objects.create_user('2', 'hazcancino@gmail.com', 'Loul')
        
        if not User.objects.filter(username='3').exists():
            User.objects.create_user('3', 'hazcancino@gmail.com', 'Loul')
        
        if not User.objects.filter(username='4').exists():
            User.objects.create_user('4', 'hazcancino@gmail.com', 'Loul')
